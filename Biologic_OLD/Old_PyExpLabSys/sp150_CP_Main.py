...
"""CP example"""'''
'''A program to run a typical CP experiment and export the data to a .csv and .json file'''
...
import numpy
import json
from bio_logic import SP150, CP
...
def run_cp():
    ...
    # technique.
    cp = CP(current_step = ([50E-6,100E-6,500E-6]), vs_initial = ([False,False,False]),
                 duration_step = ([2,2,2]),
                 record_every_dT = 0.1, record_every_dE = 5E-3,
    ...
    df = (Time, Ewe, I, cycle)
    numpy.savetxt("SP150_CP_Main.csv", numpy.transpose(df), delimiter=",", header = 'Time,Ewe,I,cycle', comments = '')

    #The second method is saving the data as a .json file
    data = {'Time':Time.tolist(),'Ewe':Ewe.tolist(),'I':I.tolist(),'cycle':cycle.tolist()}
    with open('SP150_CP_Main.json', 'w') as json_file:
