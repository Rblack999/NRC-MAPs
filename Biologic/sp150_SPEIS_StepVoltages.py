...
def run_speis():
    ...
    #Blank object for filling of the data into a dictionary
    dict = []
    ...
    for i in [0.1,0.2,0.3]:
        speis = SPEIS(vs_initial = False, vs_final = False, initial_voltage_step = i,
                     final_voltage_step = i, duration_step = 0.5, step_number = 1,
    ...
    #Better way to add this to the dictionary? Have each cycle run be at a specific voltage? Just have the data be outputted to dictionaries with hierarchy of the voltage?
        ...
        numpy.savetxt(f"SP150_SPEIS_Main{i}.csv", numpy.transpose(df), delimiter=",", header = 'freq,abs_Ewe,abs_I,Phase_Zwe,abs_Ece,abs_Ice,Phase_Zce,Ece', comments = '')

        # The second method is saving the data as a .json file
        data= {f'{i} V':{'freq':freq.tolist(),'abs_Ewe':abs_Ewe.tolist(),'abs_I':abs_I.tolist(),'Phase_Zwe':Phase_Zwe.tolist(),'abs_Ece':abs_Ece.tolist(),
                'abs_Ice':abs_Ice.tolist(),'Phase_Zce':Phase_Zce.tolist(),'Ece':Ece.tolist()}}
        dict.append(data)
        with open(f'SP150_SPEIS_Main{i}.json', 'w') as json_file:
