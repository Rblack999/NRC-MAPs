e
from north_c9 import NorthC9  # import the class to communicate with the C9 controller
...
print("A position of", counts_value, "maps to an angle of", rad_value, "for the SHOULDER")
rad_value = -pi/4
counts_value = c9.rad_to_counts(c9.ELBOW, rad_value)
print("An of", rad_value, "maps to a position of", counts_value, "for the ELBOW")
...
# For now, the inverse kinematics solver returns the solution with the "upper arm" link
# between the shoulder and elbow joints aligned as closely with the task-space +Y axis
# as possible to minimize collisions. In a near-future release there will be an option to
# toggle this, as well as improved IK for a variety of tools.

c9.move_axis_mm(c9.Z_AXIS, 100)  # moves so that the bottom of the grippers are 100mm above the deck
c9.move_xy (-150, 0)  # move the elbow and shoulder so that the gripper is at the given xy
c9.move_xyz (-225, 18.75, 80) # move elbow, shoulder, and z_axis to the given task space coordinate
...
# The goto() method accepts a list of counts in this format and moves the robot there.
c9.goto(test_point)
...
# 1.5-2 revs to fully disengage the threads, at 10000 cts/sec accelerating at 50000 cts/sec^2:
...
c9.uncap(1.5, 2, 1750, 5000, 20000)
