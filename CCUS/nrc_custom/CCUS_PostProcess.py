...
class PostProcessing:
    ...
    def curate_deposition(self): 
        ...
        for column in df.columns:
            ...
            self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['OCV'] = ocv_data
        ...
        for column in df.columns:
            ...
            self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['PEIS'] = peis_data

        # Get the iR value from the initial PEIS scan
        data_iR = self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['PEIS']
        ...
        self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['PEIS']['iR(ohm)'] = iR
        ...
        for column in df.columns:
            ...
            self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['CA'] = ca_data
        # Add a ['Ewe(V)_corrected'] to take into account iR correction
        self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['CA']['Ewe(V)_corrected'] = ca_data['Ewe(V)']-(np.array(ca_data['I(A)'])*iR)
    ...
    def curate_characterization(self):
        ...
        for i in [2,10]:
            ...
            for column in df.columns:
                peis_data[column] = df[column].to_list()
                self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char'][f'PEIS_{i}'] = peis_data
        
            # Get the iR value from the initial PEIS scan
            data_iR = self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char'][f'PEIS_{i}']
            index = [abs(i) for i in data_iR['Phase_zwe']].index(min((abs(i)) for i in data_iR['Phase_zwe']))
            E_phase0 = np.array(data_iR['Ewe_bar(V)'][index])
            I_phase0 = np.array(data_iR['I_bar(A)'][index])
            iR = E_phase0/I_phase0
            print(f'iR for deposition = {iR} ohms')
            self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char'][f'PEIS_{i}']['iR(ohm)'] = iR
        ...
        for i in [3,4,5,6,7,8,9]:
            ...
            for column in df.columns:
                ...
                self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char'][f'CV_{i}'] = cv_data
    ...
    def curate_performance_proxy(self):
        ...
        for column in df.columns:
            ca_data[column] = df[column].to_list()
            self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char']['CA'] = ca_data

        # Plot the data
        x = self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char']['CA']['t(s)']
        y = self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char']['CA']['I(A)']
        plt.plot(x,y)
    ...
    def determine_mass_loading(self, truncate_time):
        ...
        # Entire deposition
        x_all = self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['CA']['t(s)']
        y_all = np.abs(self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['CA']['I(A)'])
        area_all = np.trapz(y_all, x_all)
        print(f'Mass Loading All = {area_all}')

        # Time truncated deposition
        filter = np.where(np.array(self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['CA']['t(s)']) > truncate_time)
        x_trunc = np.array(self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['CA']['t(s)'])[filter]
        y_trunc = np.array(np.abs(self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Depo']['CA']['I(A)']))[filter]
        ...
        self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Metric']['Loading'] = {'mass_total':area_all,'mass_trunc':area_trunc,'trunc_time':truncate_time}
    ...
    def determine_ESCA(self):
        ...
        for i,j in enumerate([4,5,6,7,8,9]): # This can be adjusted based no the number of scan_rates used, note did not use the first CV
            Ewe_array = np.array(self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char'][f'CV_{j}']['Ewe(V)'])
            current_array = np.array(self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char'][f'CV_{j}']['I(A)'])
            cycle_array = np.array(self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char'][f'CV_{j}']['Cycle'])
    ...
    def determine_performance_proxy(self):
        ...
        # For this proxy experiment, just take the last current value at the end of the experiment. Note the actual current values are negative, so we want them to be positive
        co2r_current = -self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Char']['CA']['I(A)'][-1]

        print(f'The current at the end of the run is {co2r_current} A')

        self.loaded_data[f'{self.experiment_name}'][f'{self.test}']['Metric']['CO_Eff'] = co2r_current 
        # Store the values as metric values in the .json file
    ...
    def experiment_update(self, exp_count, X_choice):
