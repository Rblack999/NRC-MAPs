...
class ActiveLearning:
    ...
    def determine_first_experiment(self,dict):
        ...
        dict[f'{self.experiment_name}'][f'Test_{self.exp_count}']['AL']['X_matrix'] = X_matrix
        # This one will always store the up-to-date chosen X matrix
        dict[f'{self.experiment_name}'][f'Test_{self.exp_count}']['AL']['X_sample'] = X_samples
    ...
    def determine_next_experiment(self, dict):
        ...
        for i in range(self.exp_count+1):
            y[i] = np.array([dict[f'{self.experiment_name}'][f'Test_{i}']['Metric']['OP']])

        Y_samples = -y[:, None] # the data object should have columns
        print(Y_samples)
        ...
        print(f'Composition = {X_samples}')
        
        #xxx is the pool that we pick the new candidate from
        ...
        xx = np.array(dict[f'{self.experiment_name}'][f'Test_0']['AL']['X_matrix'])
        xxx = np.zeros(xx.shape)
        xxx = xx.astype(np.float32) 
        ...
        def next_points(init_x , init_y , best_init_y ,xxx, n_points):
        ...
        candidates= next_points(init_x, init_y, best_init_y, xxx, 1)
        ...
        index= np.where(np.all(xxx==cand,axis=1))
